	...
	function FcEvent(vevent, event, start, end, recurrenceDetails) {
		...
		iface.createFork = function() {
			...
			if (oldRRule.count) {
				...
				console.log('new', newRRule.count);
				console.log('old', oldRRule.count);
			} else {
				oldRRule.until = iface.recurrenceDetails.recurrenceId.clone();
				oldRRule.until.addDuration(ICAL.Duration.fromSeconds(-1));
			}

			context.previousEvent.updatePropertyWithValue('rrule', oldRRule);
			newComp.updatePropertyWithValue('rrule', newComp);

			console.log(context.previousEvent, newComp);
